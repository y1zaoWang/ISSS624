...
```{r}
hunan <- st_read(dsn = "Hands-on-Ex2/data/geospatial", 
                 layer = "Hunan")
```
...
```{r}
hunan2012 <- read_csv("Hands-on-Ex2/data/aspatial/Hunan_2012.csv")
```
...
The "Average number of links: 3.681818" in the context of a neighbor list object typically refers to the average number of connections or relationships each region has with other regions in a network or graph. This average is calculated by dividing the total number of nonzero links (which are the actual connections between different regions) by the total number of regions.

In this specific case, with 88 regions and 324 nonzero links, the average is calculated as 324 divided by 88, which gives the average number of links as approximately 3.68. This means that on average, each region is directly connected to about 3.68 other regions.
