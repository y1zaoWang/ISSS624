...
```{r}
mpsz <- read_rds("In-class_Ex3/data/rds/mpsz.rds")
mpsz
```
...
```{r}
write_rds(distPair, "In-class_Ex3/data/rds/distPair.rds") 
```
...
```{r}
od_data <- read_rds("In-class_Ex3/data/rds/od_data.rds")
```
...
```{r}
pop <- read_csv("In-class_Ex3/data/aspatial/pop.csv")
```
...
```{r}
write_rds(flow_data1, "In-class_Ex3/data/rds/SIM_data")
```
...
```{r}
SIM_data <- read_rds("In-class_Ex3/data/rds/SIM_data.rds")
```
...
![The code chunk used to calibrate to model is shown below:](images/image1.jpg)
